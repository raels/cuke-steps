...
class StepParser
  ...
  def parse_lines
    ...
    while not @lines.empty?
      ...
      when /^ *#/
        @comments << line
      when /^(Given|When|Then|And|But|Before|After|AfterStep|Transform)[ (]/
        unread(line)
        parse_step
        @comments = []
      when /^\s+(Given|When|Then|And|But|Before|After|AfterStep|Transform)[ (]/
  ...
  def parse_step_name(line)
    line = line.sub(/^(Given|When|Then|And|But|Transform) *\(?\/\^?(.*?)\$?\/.*/, '\1 \2')
    line = line.gsub('\ ', ' ')
    line
  end
